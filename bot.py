...
EMOJI = {
    ...
    "done": "✅", "error": "❌", "wait": "⏳", "arrow": "➜",
    "bot": "🤖", "info": "ℹ️", "star": "⭐", "scissors": "✂️",
    "compress": "🗜", "resize": "📐", "mute": "🔇", "thumb": "🖼",
    "merge": "📝", "stats": "📊",
}
...
def format_buttons(file_type):
    formats_map = {"image": IMAGE_FORMATS, "video": VIDEO_FORMATS, "audio": AUDIO_FORMATS, "doc": DOC_FORMATS}
...
def get_file_info(src):
    ...
    try:
        ...
        info = {"size": int(fmt.get("size", 0)), "duration": float(fmt.get("duration", 0)), "bitrate": int(fmt.get("bit_rate", 0)), "format": fmt.get("format_long_name", "Unknown")}
        if video_stream:
            ...
            try: info["fps"] = eval(video_stream.get("r_frame_rate", "0/1"))
            except: info["fps"] = 0
        if audio_stream:
...
async def cmd_help(update, ctx):
    text = (
        ...
        "*Tools:*\n"
        "✂️ Trim — cut video to start/end times e.g. 0:10\n"
        "📐 Resize — set dimensions e.g. 1280x720\n"
        "🗜 Compress — reduce file size\n"
        "🔇 Mute — remove audio from video\n"
        "🖼 Thumbnail — extract a frame at a timestamp\n"
...
async def cmd_formats(update, ctx):
    text = (
        "⭐ *Supported Formats*\n\n"
        "🖼 *Image:* JPG PNG WEBP BMP GIF TIFF ICO → + PDF\n"
        "🎬 *Video:* MP4 AVI MKV MOV WEBM FLV → MP4 AVI MKV MOV WEBM GIF MP3 AAC WAV VIDNOTE\n"
        "🎵 *Audio:* MP3 WAV AAC OGG FLAC M4A OPUS → same\n"
...
async def handle_file(update, ctx):
    ...
    type_labels = {"image": "🖼 Image", "video": "🎬 Video", "audio": "🎵 Audio", "doc": "📄 Document"}
...
async def handle_text(update, ctx):
    ...
    if tool == "trim_waiting_start":
        ...
        await update.message.reply_text("Now send the *end time* (e.g. `1:30`):", parse_mode=ParseMode.MARKDOWN)
        return
    ...
    if tool == "resize_waiting":
        try:
            ...
            await update.message.reply_text("❌ Send dimensions like `1280x720`", parse_mode=ParseMode.MARKDOWN)
        return
...
async def handle_callback(update, ctx):
    ...
    if query.data == "do_merge":
        ...
        try:
            ...
            out_size = out_path.stat().st_size
            with open(out_path, "rb") as f:
                await query.message.reply_document(document=f, caption="✅ Merged PDF ready! (" + str(out_size // 1024) + " KB)")
    ...
    if query.data.startswith("tool:"):
        ...
        if tool == "trim":
            ...
            await query.edit_message_text("✂️ *Video Trim*\n\nSend the *start time* (e.g. `0:10` or `1:30`):", parse_mode=ParseMode.MARKDOWN)
            return

        if tool == "resize":
            ctx.user_data["tool"] = "resize_waiting"
            await query.edit_message_text("📐 *Resize Image*\n\nSend dimensions as `widthxheight`\n\nExamples:\n• `1280x720`\n• `800x0` for auto height\n• `0x1080` for auto width", parse_mode=ParseMode.MARKDOWN)
            return

        if tool == "thumbnail":
            ctx.user_data["tool"] = "thumbnail_waiting"
            await query.edit_message_text("🖼 *Extract Thumbnail*\n\nSend the timestamp (e.g. `0:05` or `1:23`):", parse_mode=ParseMode.MARKDOWN)
            return

        if tool == "compress":
            if file_type == "image":
                ctx.user_data["tool"] = "compress_waiting"
                await query.edit_message_text("🗜 *Compress Image*\n\nSend quality (1-100):\n• 80 = high quality\n• 60 = balanced\n• 40 = smaller size", parse_mode=ParseMode.MARKDOWN)
                return
            else:
        ...
        if tool == "merge_pdf":
            ...
            await query.edit_message_text("📝 *PDF Merge Mode*\n\nSend PDFs one by one. Tap merge when ready.", parse_mode=ParseMode.MARKDOWN)
            return
    ...
    if query.data.startswith("convert:"):
        ...
        await query.edit_message_text("⏳ Converting " + filename + " to " + target_fmt + "...", parse_mode=ParseMode.MARKDOWN)
        ...
        try:
            ...
            if not converter:
                raise ValueError("No converter for type: " + str(file_type))
        ...
        try:
            out_size = out_path.stat().st_size
            caption = "✅ Converted! `" + out_path.name + "` (" + str(out_size // 1024) + " KB)"
            with open(out_path, "rb") as f:
                ext = out_path.suffix.lower()
